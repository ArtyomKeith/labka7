...
# Загружаем данные
file_path = st.file_uploader("Загрузите файл с данными (формат Excel):", type=["xlsx"])
if file_path:
    ...
    if not filtered_data.empty:
        ...
        color_choice = st.color_picker("Выберите цвет для графиков", "#1f77b4")

        # Линейный график
        with tab1:
            st.write("График ниже показывает изменение {} по годам для выбранных стран.".format(metrics_mapping[selected_metric]))
            plt.figure(figsize=(12, 6))
            sns.lineplot(data=filtered_data, x='year', y=selected_metric, hue='Страна', marker='o', linewidth=2.5, palette=[color_choice])
        ...
        with tab2:
            ...
            plt.figure(figsize=(12, 6))
            sns.barplot(data=filtered_data, x='year', y=selected_metric, hue='Страна', ci=None, palette=[color_choice])
